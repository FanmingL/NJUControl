...
void LCD_FSMC_Config()
{
  ...
  FSMC_NORSRAMTimingInitTypeDef  readWriteTiming; 
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOE, ENABLE);//使能PD,PE,PF,PG时钟  
 ...
 GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;    //推挽输出
  ...
  GPIO_Init(GPIOE, &GPIO_InitStructure);            //初始化  
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;         //PG2,FSMC_A12   by ycnalin FSMC_A16~PD11
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;      //复用输出
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;    //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;      //上拉
  GPIO_Init(GPIOD, &GPIO_InitStructure);            //初始化  
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;        //PG12
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;      //复用输出
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;    //推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;      //上拉
  GPIO_Init(GPIOD, &GPIO_InitStructure);            //初始化   //LCD CS   by ycnalin PG12_NE4 ~PD7_FSMC_NE1
  ...
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;      //上拉
  GPIO_Init(GPIOE, &GPIO_InitStructure);            //				//RST
  ...
  GPIO_PinAFConfig(GPIOE,GPIO_PinSource0,GPIO_AF_FSMC);  //LCD_RST    by ycnalin
 
  GPIO_PinAFConfig(GPIOD,GPIO_PinSource11,GPIO_AF_FSMC);//FSMC_A16  by ycnalin   
  GPIO_PinAFConfig(GPIOD,GPIO_PinSource7,GPIO_AF_FSMC); // LCD_CS 对应FSMC_NE1   by ycnalin
  ...
  FSMC_NORSRAMInitStructure.FSMC_Bank = FSMC_Bank1_NORSRAM1;//   使用NE4 ，也就对应BTCR[6],[7]。
  FSMC_NORSRAMInitStructure.FSMC_DataAddressMux = FSMC_DataAddressMux_Disable; // 不复用数据地址
  FSMC_NORSRAMInitStructure.FSMC_MemoryType =FSMC_MemoryType_SRAM;// FSMC_MemoryType_SRAM;  //SRAM 与NOR  均可 
  ...
  FSMC_NORSRAMCmd(FSMC_Bank1_NORSRAM1, ENABLE);  // 使能BANK1   FSMC_NE1
...
{ 			
	...
	//软件复位。若使用硬件复位，则PE0不接即可,LCD—RST接芯片复位口
